...
class AddMovie(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        blank = "Cannot add movie without title"
        No = "{0} is a terrible movie. It will not be added to your watchlist.".format(new_movie)
        if new_movie == "":
            self.redirect("/?error=" + blank)
        if new_movie in terrible_movies:
            self.redirect("/?error=" + No)
